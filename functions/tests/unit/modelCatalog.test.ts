...
const EXISTING_METADATA: Record<string, Omit<ModelInfo, 'id'>> = {
  ...
  'grok-4.5': { name: 'Grok 4.5', description: 'Frontier general reasoning model', quality: 5, speed: 'Medium', cost: '★☆☆', supportsMultimodal: false, contextLength: '256K', bestFor: 'Reasoning, general tasks' },
...
describe('OpenCode Go model catalog contract', () => {
  it('contains the canonical 23 models in fixed protocol order', () => {
    expect(MODEL_CATALOG.map(({ info, protocol }) => [info.id, protocol])).toEqual(EXPECTED_MODELS);
    expect(new Set(MODEL_CATALOG.map(({ info }) => info.id)).size).toBe(23);
  });

  it('contains 3 Responses, 13 Chat Completions, and 7 Messages models', () => {
    const counts = MODEL_CATALOG.reduce<Record<string, number>>((result, model) => {
      result[model.protocol] = (result[model.protocol] ?? 0) + 1;
      return result;
    }, {});

    expect(counts).toEqual({ responses: 3, 'chat-completions': 13, messages: 7 });
  });

  it('keeps the public metadata of the existing 18 models', () => {
    expect(Object.keys(EXISTING_METADATA)).toHaveLength(18);
    for (const [id, metadata] of Object.entries(EXISTING_METADATA)) {
      expect(getModelConfig(id)?.info).toEqual({ id, ...metadata });
    }
  });

  it('uses neutral metadata for the five new models', () => {
    expect([
      ...
      getModelConfig('ox-alpha-free')?.info,
      ...
      { id: 'ox-alpha-free', name: 'Ox Alpha Free', description: 'OpenCode Go model. Limited-time availability.', quality: 3, speed: 'Unknown', cost: 'See OpenCode Go', supportsMultimodal: false, contextLength: 'Unknown', bestFor: 'General use' },
  ...
  it('provides the default and lookup helpers without accepting excluded models', () => {
    ...
    expect(getModelProtocol('grok-4.5')).toBe('responses');
    expect(getModelProtocol('glm-5.2')).toBe('chat-completions');
    expect(getModelProtocol('minimax-m3')).toBe('messages');

    for (const id of ['kimi-k2.5', 'glm-5', 'qwen3.5-plus', 'mimo-v2-pro', 'mimo-v2-omni', 'hy3-preview']) {
