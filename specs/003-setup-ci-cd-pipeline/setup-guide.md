...
> **注意**: `--client-id` と `--tenant-id` の値は、手順 1 の出力からコピーしてください。`--swa-deployment-token` は Azure Portal の Static Web App リソースから取得します。
...
| 名前 | 値 | 説明 |
|------|-----|------|
| `AZURE_CLIENT_ID` | `setup-federated-credentials.sh` の出力（clientId） | Azure SP クライアント (App) ID |
| `AZURE_TENANT_ID` | `setup-federated-credentials.sh` の出力（tenantId） | Azure AD テナント ID |
...
| `SWA_DEPLOYMENT_TOKEN` | Static Web App のデプロイトークン | Azure Static Web Apps デプロイ用 |
...
| `AADSTS700016: Application not found` | App ID が一致していない | `AZURE_CLIENT_ID` の値が正しいか確認 |
